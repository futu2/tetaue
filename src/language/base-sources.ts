...
# (\`isNothing\`, \`isJust\`, \`fromMaybe\`); the constructors and the
# \`Maybe.*\` namespace spellings come from the core.

export isNothing = isNull
export isJust = x => not (isNull x)
export isNotNull = isJust
`;
...
# Aliases
#
# A second spelling for an operation whose lowered SQL differs by ARGUMENT
# (not by definition): \`rpad\` is \`lpad\` spelled the other way, and its
# lowering reads the written name to decide the direction. Kept as ordinary
# aliases so both spellings resolve to one implementation.
# ---------------------------------------------------------------------------

export isNotIn = core.isNotIn
export least = core.least
export rpad = core.rpad
...
export concat = core.concat
export greatest = core.greatest
export substring = core.substring
export round = core.round
export lpad = core.lpad
export coalesce = core.coalesce
export nullIf = core.nullIf
...
export isNull = core.isNull
export maybeIsJust = core.maybeIsJust
export isTrue = core.isTrue
export isFalse = core.isFalse
export isUnknown = core.isUnknown
