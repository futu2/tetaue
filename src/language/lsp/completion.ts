...
const STEP_NAMES = new Set(['filter', 'map', 'sort', 'take', 'distinct', 'fold', 'joinInner', 'joinLeft', 'joinRight', 'joinFull', 'joinLateral', 'select']);
...
export class TetaueCompletionProvider extends DefaultCompletionProvider {
    ...
    constructor(services: TetaueServices) {
        ...
        // The suggested names are the PRELUDE'S SURFACE, not the primitive
        // core: `sql_func`/`op_add` and friends are library-internal and a
        // user module cannot name them, so offering them would suggest code
